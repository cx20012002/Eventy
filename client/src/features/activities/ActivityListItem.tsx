    ...
    return (
        <div className={"rounded flex md:gap-8 gap-3"}>
            ...
            <div className={"flex flex-col basis-7/12"}>
                ...
                <hr className={"my-2"}/>
                <span className={"text-xs text-gray-500"}>Hosted by {activity.host?.displayName}</span>
                {activity.isHost && (
            ...
            <div className={"text-right flex-col flex basis-2/12"}>
                <div
                    className={"text-sm font-medium text-red-300 bg-red-100 w-fit self-end rounded text-center px-5 py-1"}>Canceled
                </div>
                <div className={"mt-auto gap-2 flex"}>
                    ...
                    <Link to={`/activities/${activity.id}`}
                          className={"px-5 py-2 bg-primary text-white text-sm text-center rounded"}>View</Link>
                </div>
            </div>
        </div>
    )
